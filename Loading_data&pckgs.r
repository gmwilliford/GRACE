...
#MONTHLY GRACE#
setwd("D:/Research/MASTER DATA/GRACE NC/MASTER/Combined")

#list to hold clipped GRACE files
grace_clipped_list <- list()
grace_files <- list.files(path = ".", pattern = "GRD.*\\.nc4$", full.names = TRUE)
...
for (i in 1:length(grace_files)) {
  ...
  shapefile_proj <- st_transform(shapefile, st_crs("+proj=longlat +datum=NAD83 +no_defs"))
...
folder_path <- "D:/Research/MASTER DATA/GLDAS/1x1/MASTER"
...
folder_path025 <- "D:/Research/MASTER DATA/GLDAS/0.25x0.25/MASTER"
